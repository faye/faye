...
JS.ENV.ClientSpec = JS.Test.describe("Client", function() { with(this) {
  ...
  describe("handshake advice on connect should rehandshake exactly once", function() { with(this) {
    before(function() { with(this) {
      createConnectedClient()
    }})

    it("reconnects exactly once", function() { with(this) {
      client.handshake()

      stub(transport, "send", function(envelope, callback, context) {
        if(callback) {
        ...
        if(message.channel === '/meta/connect') {
          client.receiveMessage({ channel: message.channel, successful: false, id: message.id, advice: {reconnect: "handshake", interval: 100 }})
        }
      });

      client.connect();

      expect(client, "connect").exactly(1)
